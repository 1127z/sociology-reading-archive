    ...
    <main><SiteHeader />
      <article className="article-shell shell">
        ...
        <div className="article-layout">
          <nav className="article-toc"><span>本篇目录</span>{["研究问题", "选题与结构", "理论框架", "研究方法", "论证链条", "主要结论", "亮点与局限", "思考题"].map((x, i) => <a href={`#s${i + 1}`} key={x}>{String(i + 1).padStart(2, "0")} {x}</a>)}</nav>
          <div className="article-content">
            ...
            <section id="s2"><span className="section-num">02</span><h2>选题来源与文章结构</h2><p>{article.selectionSource ?? "文章从既有文献的解释缺口与现实经验问题之间建立连接。"}</p>{article.articleStructure && <ol>{article.articleStructure.map((x) => <li key={x}>{x}</li>)}</ol>}</section>
            ...
            <section id="s4"><span className="section-num">04</span><h2>数据与研究方法</h2><ul>{article.methods.map((x) => <li key={x}>{x}</li>)}</ul></section>
            <section id="s5"><span className="section-num">05</span><h2>分析思路与论证链条</h2><ol className="argument-chain">{article.chain.map((x, i) => <li key={x.label}><b>{String(i + 1).padStart(2, "0")}</b><div><h3>{x.label}</h3><p>{x.detail}</p></div></li>)}</ol></section>
            <section id="s6"><span className="section-num">06</span><h2>主要结论</h2><ul>{article.findings.map((x) => <li key={x}>{x}</li>)}</ul></section>
            <section id="s7"><span className="section-num">07</span><h2>亮点与可批判之处</h2><div className="highlight-list">{article.highlights.map((x) => <div key={x.label}><b>{x.label}</b><p>{x.detail}</p></div>)}</div><h3 className="subhead">研究局限</h3><ul>{article.limits.map((x) => <li key={x}>{x}</li>)}</ul></section>
            <section id="s8"><span className="section-num">08</span><h2>阅读思考题</h2><ol className="questions">{article.questions.map((x, i) => <li key={x}><b>{i + 1}</b><p>{x}</p></li>)}</ol></section>
