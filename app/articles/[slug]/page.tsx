    ...
    <main><SiteHeader />
      <article className="article-shell shell">
        <header className="article-hero">
          ...
          <aside><span className="eyebrow">PUBLICATION</span><strong>{article.journal}</strong><p>{article.year} · {article.volume} · {article.pages}</p>{article.evidenceBasis && <p>分析依据：{article.evidenceBasis} · {article.analysisDepth}<br />证据置信度：{article.confidence}</p>}<a href={article.sourceUrl} target="_blank" rel="noreferrer">阅读官方原文 ↗</a><a className="doc-link" href={article.documentUrl}>下载精读文档 ↓</a><ReadingToggle slug={article.slug} /></aside>
        </header>
